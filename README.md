...
| Mode | What you do |
|---|---|
| **Layout** | `+ Cubicle` / `+ Office` add the two main seat types; `+ Other…` opens the full space list (Conf. Room, Reception, Huddle, Break Room, Warehouse, Server Room, Storage, Restroom, Copy/Print, Door). Drag to move (snaps to an 8px grid). Tap to select — then drag the blue corner dot to resize, or use Rename / Duplicate / Delete. Undo covers all layout changes. |
...
- **Site name** in the header opens the site picker — alphabetically sorted, scrolls past ~10 sites. The **gear** opens Settings: new/rename/duplicate/delete site and JSON **Export / Import**.
